...
- `ALERT_USERNAME` - необязательное имя или список имён через запятую, используемый в приветствии при отложенной публикации; позиции соответствуют `PROMOTIONS_CHANNEL_ID`; если позиция пустая, приветствие для соответствующего канала начинается с `Привет!`
- `PROMOTIONS_CHANNEL_ID` - id Telegram-канала или `@channel_username`; можно указать несколько значений через запятую; если задан, включает публикацию акций по расписанию
...
Если задан `PROMOTIONS_CHANNEL_ID`, бот публикует тот же контент, что и по `/акции`, в указанный Telegram-канал или список каналов раз в неделю в день и время, заданные в `PROMOTIONS_SCHEDULE_TIME`.
Несколько каналов указываются через запятую, например `PROMOTIONS_CHANNEL_ID=@first_channel,-1001234567890,@third_channel`.
Имена для приветствий можно указать позиционно: `ALERT_USERNAME=@first_alert,,@third_alert` означает имя для первого канала, пустое имя для второго и имя для третьего.
...
- `Приветствую <ALERT_USERNAME>!`, а затем `Вот список акционных товаров на новой неделе.`, если для позиции канала задан `ALERT_USERNAME`
- `Привет!`, а затем `Вот список акционных товаров на новой неделе.`, если позиция `ALERT_USERNAME` пустая или отсутствует
...
Статус публикации хранится отдельно для каждого канала, поэтому при временной ошибке одного канала scheduler повторит только неуспешную отправку.
